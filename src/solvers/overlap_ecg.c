...
CPLM_PUSH
  int rank, ierr = 0;
  // int nCol = 0; not necessary any more
  double trash_t;
  // Reset timings
  ecg->tot_t  = 0.E0;
  ecg->comm_t  = 0.E0;
  ...
  *normb_p = sqrt(*normb_p);
  ...
  // nCol = rank % t; not necessary any more
  // int size;
  // MPI_Comm_size(ecg->comm,&size);
  // nCol = (int) (rank * t / size);
  ierr = _preAlps_oECGSplit(rhs, R, glob_indices);
...
CPLM_PUSH
  ...
  // Allocate Memory
  ierr = _preAlps_oECGMalloc(ecg);
  // Set to zero the parameters and split rhs
  ierr = _preAlps_oECGReset(ecg,rhs,rci_request,glob_indices);
...
CPLM_PUSH
  ...
  for (int i = 0; i < XSplit->info.m; i++) {
    colIndex = glob_indices[i] % enl_factor;
...
CPLM_PUSH
  ...
  // computing the residual norm - NEW by Jan Papez, April 2019
  ...
  *res_p = 0.E0;
  for(int i = 0; i < m; i++) {
    *res_p += residual.val[i] * residual.val[i] * w[i];
  }
  trash_t = MPI_Wtime();
  ierr = MPI_Allreduce(MPI_IN_PLACE,res_p,1,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
  ecg->comm_t += MPI_Wtime() - trash_t;
  *res_p = sqrt(*res_p);
  free(residual.val);
  ...
  ierr = _preAlps_oECGweightedMatMult(R, R, w, &RtR_s);
...
CPLM_PUSH
  ...
  int ierr = 0;
  if (ecg->ortho_alg == ORTHOMIN)
    _preAlps_oECGIterateOmin(ecg, rci_request, w);
  else if (ecg->ortho_alg == ORTHODIR)
    _preAlps_oECGIterateOdir(ecg, rci_request, w);
  else if (ecg->ortho_alg == ORTHODIR_FUSED)
    _preAlps_oECGIterateOdirFused(ecg, rci_request, w);
...
CPLM_PUSH
  ...
  if (*rci_request == 0) {
    ...
    ierr = _preAlps_oECGweightedMatMult(AP, P, w, &work_s);
    ecg->gemm_t += MPI_Wtime() - trash_t;
    trash_t = MPI_Wtime();
    ...
    ierr = _preAlps_oECGweightedMatMult(P, R, w, alpha);
    ecg->gemm_t += MPI_Wtime() - trash_t;
    trash_t = MPI_Wtime();
    ...
    *rci_request = 1;
    ...
    ierr = _preAlps_oECGweightedMatMult(AP, Z, w, beta);
    ecg->gemm_t += MPI_Wtime() - trash_t;
    trash_t = MPI_Wtime();
    ...
    if (ecg->bs_red == ADAPT_BS) {
      ...
      ierr = _preAlps_oECGweightedMatMult(P, P, w, &work_s);
      ecg->gemm_t += MPI_Wtime() - trash_t;
      // Sum local dot products in place (no mem alloc needed)
      trash_t = MPI_Wtime();
      ierr = MPI_Allreduce(MPI_IN_PLACE, work_s.val, work_s.info.nval,
...
//                            nrhs,iwork,tau_p);
    ...
    *rci_request = 1;
    ...
    ierr = _preAlps_oECGweightedMatMult(AV, Z, w, beta);
    ecg->gemm_t += MPI_Wtime() - trash_t;
    trash_t = MPI_Wtime();
    ierr = MPI_Allreduce(MPI_IN_PLACE, beta->val, beta->info.nval,
                         MPI_DOUBLE, MPI_SUM, comm);
...
int _preAlps_oECGIterateOdirFused(preAlps_ECG_t* ecg, int* rci_request, double* w) {
  CPLM_PUSH
    ...
    ierr = _preAlps_oECGweightedMatMult(AV, Z, w, beta);
    //ierr = CPLM_MatDenseKernelMatDotProd(AV, Z, beta);
    ierr = _preAlps_oECGweightedMatMult(AP, P, w, &mu_s);
    //ierr = CPLM_MatDenseKernelMatDotProd(AP, P, &mu_s);
    ierr = _preAlps_oECGweightedMatMult(P, R, w, &rtr_s);
    //ierr = CPLM_MatDenseKernelMatDotProd(R, R, &rtr_s);
    ecg->gemm_t += MPI_Wtime() - trash_t;
    trash_t = MPI_Wtime();
...
CPLM_PUSH
  int ierr = _preAlps_oECGWrapUp(ecg, solution);
  _preAlps_oECGFree(ecg);
CPLM_POP
  return ierr;
}
...
// the function for performing the weighted matrix-matrix multiplication
...
int loop_index_1, loop_index_2;
if (X->info.stor_type == ROW_MAJOR) {
  loop_index_1 = X->info.n;
  loop_index_2 = 1;
}
else {
  loop_index_1 = 1;
  loop_index_2 = X->info.m;
}
...
for(int i=0; i < t1; i++){
    for(int j=0; j < t2; j++){
      C->val[i + j*t1] = 0.0;
      for(int k=0; k < X->info.m; k++){
        C->val[i + j*t1] += w[k] * X->val[k*loop_index_1 + i*loop_index_2] * Y->val[k*loop_index_1 + j*loop_index_2];
      }
    }
  }
